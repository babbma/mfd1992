package com.app.entity.easyui;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

/**
 * EasyUI tree模型
 */
public class Tree{

	private int id;						//节点的ID
	private String text;				//节点显示的文字
	private String state = "open";		//默认open,当为‘closed’时说明此节点下有子节点，否则此节点为叶子节点
	private boolean checked = false;	//指示节点是否被选中
	private Object attributes;			//给一个节点追加的自定义属性
	private List<Tree> children;		//定义了一些子节点的节点数组
	private String iconCls;				//定义该节点的样式	
	private int pid;					//定义该节点的父节点
	
	
	public Tree() {
		super();
	}
	
	public Tree(int id, String text, int pid) {
		super();
		this.id = id;
		this.text = text;
		this.pid = pid;
	}
	public Tree(int id, String text, int pid,String state) {
		super();
		this.id = id;
		this.text = text;
		this.pid = pid;
		this.state = state;
	}

	public Tree(int id, String text,String state, Object attributes, int pid) {
		super();
		this.id = id;
		this.text = text;
		this.state = state;
		this.attributes = attributes;
		this.pid = pid;
	}
	
	public Tree(int id, String text, Object attributes, int pid) {
		super();
		this.id = id;
		this.text = text;
		this.attributes = attributes;
		this.pid = pid;
	}

	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public String getText() {
		return text;
	}
	public void setText(String text) {
		this.text = text;
	}
	public String getState() {
		return state;
	}
	public void setState(String state) {
		this.state = state;
	}
	public boolean isChecked() {
		return checked;
	}
	public void setChecked(boolean checked) {
		this.checked = checked;
	}
	public Object getAttributes() {
		return attributes;
	}
	public void setAttributes(Object attributes) {
		this.attributes = attributes;
	}
	public List<Tree> getChildren() {
		return children;
	}
	public void setChildren(List<Tree> children) {
		this.children = children;
	}
	public String getIconCls() {
		return iconCls;
	}
	public void setIconCls(String iconCls) {
		this.iconCls = iconCls;
	}
	public int getPid() {
		return pid;
	}
	public void setPid(int pid) {
		this.pid = pid;
	}
	
	@SuppressWarnings("unchecked")
	public Tree setAttr(String key ,String value){
		Map<String, String> attr;
		if(attributes==null){
			attributes = new HashMap<String, String>();
		}
		attr= (Map<String, String>)attributes;
		attr.put(key, value);
		return this;
	}


}
